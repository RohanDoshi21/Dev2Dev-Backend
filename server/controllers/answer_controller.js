...
exports.createAnswer = async (req, res) => {
    let query = "insert into Answer (description, owner, question_id, upvotes, downvotes, created_at) values ($1, $2, $3, $4, $5, $6) returning *";
    let values = [req.body.description, req.user.id, parseInt(req.body.question), 0, 0, new Date()];
...
exports.updateAnswer = async (req, res) => {
    let query = "update Answer set description = $1 where id = $2 and owner = $3 returning *";
    let values = [req.body.description, parseInt(req.params.id), req.user.id];

    try {
        const data = await client.query(query, values);
        if (data.rows.length === 0) {
            return res.status(404).json({ error: "Answer not found or current user doesn't have permission to modify the answer" });
        }
        else {
            return res.json({ data: { answer: { ...data.rows[0] } } });
        }
...
exports.deleteAnswer = async (req, res) => {
    ...
    try {
        const data = await client.query(query, values);
        if (data.rows.length === 0) {
            return res.status(404).json({ error: "Answer not found or current user doesn't have permission to delete the answer" });
        }
        else {
            return res.json({ data: { answer: { ...data.rows[0] } } });
        }
